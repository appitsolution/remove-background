...
const slidesMakes = {
  ...
  switcherEl.addEventListener('click',({target})=> {
    const buttonsSwitcher = document.querySelectorAll('.makes__example-switcher-button')
    if(target.nodeName !== "BUTTON") return
    if(target.id === 'makes-1'){
      const addSlides = slidesMakes.people.map((item)=> (  
    ...
    if(target.id === 'makes-2'){
      const addSlides = slidesMakes.products.map((item)=> (  
      `<li class="makes__example-rules-item">
      <div class="makes__example-rules-item-image-block">
    ...
    if(target.id === 'makes-3'){
      const addSlides = slidesMakes.graphics.map((item)=> (  
      `<li class="makes__example-rules-item">
      <div class="makes__example-rules-item-image-block">
    ...
    if(target.id === 'makes-4'){
      const addSlides = slidesMakes.car.map((item)=> (  
      `<li class="makes__example-rules-item">
      <div class="makes__example-rules-item-image-block">
